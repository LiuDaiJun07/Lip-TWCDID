...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        return h,acc_global, acc

    def __str__(self):
        h, acc_global, acc = self.compute()
        return (
            'global correct: {:.1f}\n'
            'average row correct: {}\n'
            ).format(
    ...
    def format(self, classes: list):
        ...
        table = prettytable.PrettyTable(["class", "acc"])
        for i, class_name, per_acc in zip(range(len(classes)), classes, (acc * 100).tolist() ):
            table.add_row([class_name, per_acc])

        return 'global correct: {:.1f}\nmean correct:{:.1f}\n{}'.format(
            acc_global.item() * 100, acc.mean().item() * 100, table.get_string())
